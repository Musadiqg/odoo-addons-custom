    ...
    def get_approver(self):
        for record in self:
            ...
            if record.state not in ['draft','cancel','sent']:
                ...
                x = record.message_ids.filtered(lambda l: l.subtype_id.name in ['RFQ Done', 'RFQ Purchase Order'])
                if x:
                    record.approve_by = x.sorted(key='create_date', reverse=True)[0].author_id.name
                # else:
