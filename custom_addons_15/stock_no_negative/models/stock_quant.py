...
class StockQuant(models.Model):
    ...
    def check_negative_qty(self):
        ...
        for quant in self:
            ...
            # Current logic (commented out):
            """
            if (
                float_compare(quant.quantity, 0, precision_digits=p) == -1
                and quant.product_id.type == "product"
                and quant.location_id.usage in ["internal", "transit"]
                and quant.location_id.id not in [14, 18, 252, 259, 3, 197, 198] #QC and virtual locations, here -ve stock is allowed
            ):
                msg_add = ""
                if quant.lot_id:
                    msg_add = _(" lot '%s'") % quant.lot_id.name_get()[0][1]
                raise ValidationError(
                    _(
                        ...
                        "become negative "
                        "(%(q_quantity)s) on the stock location '%(complete_name)s'. "
                        "Negative stock is not allowed in this location."
                    )
                    % {
                        "name": quant.product_id.display_name,
                        "name_lot": msg_add,
