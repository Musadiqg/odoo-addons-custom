...
# class multiple_product_price(models.Model):
#     _name = 'multiple_product_price.multiple_product_price'
#     _description = 'multiple_product_price.multiple_product_price'
...
class Product(models.Model):
    ...
    x_x_price = fields.Float("Un-Registered Price")

class Partner(models.Model):
    _inherit = 'res.partner'

    is_registered = fields.Boolean("Is Un-Registered")
...
class SaleOrder(models.Model):
    ...
    def _get_display_price(self, product):
        ...
        if self.order_id.pricelist_id.discount_policy == 'with_discount':
            if self.order_id.company_id.id == 5:
                if self.order_id.partner_id.is_registered:
                    return product.with_context(pricelist=self.order_id.pricelist_id.id, uom=self.product_uom.id).x_x_price
