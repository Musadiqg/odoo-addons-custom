...
class StockQuant(models.Model):
    ...
    def _update_available_quantity(
        ...
        release = False
        if not location_id.allow_negative_stock and location_id.usage in ["view", "internal", "transit",
                                                                          "supplier", "customer"]:
            ...
            if lot_id:
                if not lot_id.quant_ids:
                    release = True
            # else:
            if location_id.id in [14, 214, 243,246]:
                release = True
            if result_qty < 0 and not release:
                if not lot_id:
                    err = _(
                    ...
                    ) % (product_id.qty_available, product_id.name, location_id.display_name, quantity)
                else:
                    err = _(
                        "You have chosen to avoid negative stock. %s pieces of %s are remaining in location %s, "
