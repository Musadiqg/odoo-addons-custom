...
class Picking(models.Model):
    ...
    def action_assign(self):
       ...
       # self.revert_reserve()
        ...
        moves = self.mapped('move_ids').filtered(lambda move: move.state not in ('draft', 'cancel', 'done')).sorted(  # Changed move_lines to move_ids
    ...
    def revert_reserve(self):
        for move in self.move_ids: 
            QUANTS = self.env['stock.quant'].search(
                [('product_id', '=', move.product_id.id), ('reserved_quantity', '>', 0)])
