...
class AccountPayment(models.Model):
    ...
    def create(self, vals_list):
        ...
        if res.move_id and res.move_id.ref:
            base_ref = res.move_id.ref
            if res.clearing_code:
                res.move_id.write({'ref': base_ref + "-" + res.clearing_code.name})
            elif res.clearing_code_many:
                res.move_id.write({'ref': base_ref + "-" + str(res.clearing_code_many.mapped('name'))})
        return res
...
class PurchaseOrder(models.Model):
    ...
    def action_create_invoice(self):
        ...
        # 4) Handle negative amounts by creating reversals
        moves.filtered(
            lambda m: m.currency_id.round(m.amount_total) < 0)._reverse_moves()  # Changed: Use _reverse_moves (Odoo 18 method) to create a credit note
